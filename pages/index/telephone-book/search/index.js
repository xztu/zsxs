...
Page({
  ...
  onLoad: async function (options) {
    ...
    // 加载列表
    const response = await wx.cloud.callContainer({ path: app.globalData.configs.container + '/index/telephone-book/fuzzy-search/' + encodeURI(options.name) + '?limit=20' })
    if (response.data.Message === "Success") {
      if (response.data.Total > 0) {
        ...
        wx.showToast({ title: '没有找到符合条件的联系人', icon: 'none' })
      }
    } else {
      // 处理错误
  ...
  onPullDownRefresh: async function () {
    ...
    await this.onLoad({ name: this.data.name });
  ...
  onReachBottom: async function () {
    ...
    // 加载列表
    const response = await wx.cloud.callContainer({ path: app.globalData.configs.container + '/index/telephone-book/fuzzy-search/' + encodeURI(this.data.name) + '?limit=20&page=' + this.data.currentPage })
    if (response.data.Message === "Success") {
  ...
  onShareAppMessage() {
    return {
      title: '校内电话簿 - "' + this.data.name + '" 的搜索结果',
